...
const apiBaseUrl = "https://cursos-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Instrutor</li>
          <li>✓ Controle completo de Curso</li>
          <li>✓ Controle completo de Modulo</li>
...
function PainelInstrutor({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "especialidade", "bio", "ativo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/instrutors", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/instrutors/" + editId : apiBaseUrl + "/api/instrutors";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "especialidade", "bio", "ativo"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/instrutors/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Instrutor")}</div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Instrutor</h3>
...
function PainelCurso({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [instrutorList, setInstrutorList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["instrutorId", "titulo", "descricao", "categoria", "cargaHoraria", "preco", "nivel", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/cursos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/instrutors", { headers: authHeaders(token) }).then(r => r.json()).then(setInstrutorList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/cursos/" + editId : apiBaseUrl + "/api/cursos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["instrutorId", "titulo", "descricao", "categoria", "cargaHoraria", "preco", "nivel", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["instrutor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/cursos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["titulo"] ?? "Curso")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/cursos/" + item.id, {
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              ...
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/curso" + item.id + "/320/200"} alt="Curso" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🗂️</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["titulo"] ?? "Curso")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>instrutorId</b><span>{item["instrutorId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>cargaHoraria</b><span>{item["cargaHoraria"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>instrutor</b><span>{item.instrutorId ? ((instrutorList.find(o => o.id === item.instrutorId) || {}).nome ?? ("#" + item.instrutorId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Curso</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">instrutorId</label>
      <input type="number" value={form["instrutorId"] ?? ""} onChange={e => setForm({...form, instrutorId: e.target.value})} />
      ...
      <label className="field-label">cargaHoraria</label>
      ...
      <select value={form["instrutor"] ?? ""} onChange={e => setForm({...form, instrutor: e.target.value})}>
        <option value="">Selecione...</option>
        {(instrutorList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelModulo({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [busca, setBusca] = React.useState("");
  const [pagina, setPagina] = React.useState(1);
  const [cursoList, setCursoList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["cursoId", "titulo", "descricao", "ordem", "duracao", "tipo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/modulos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/cursos", { headers: authHeaders(token) }).then(r => r.json()).then(setCursoList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/modulos/" + editId : apiBaseUrl + "/api/modulos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["cursoId", "titulo", "descricao", "ordem", "duracao", "tipo"].forEach(k => { f[k] = item[k] ?? ""; });
    ["curso"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/modulos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["titulo"] ?? "Modulo")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>cursoId</b><span>{item["cursoId"] ?? "—"}</span></div>
            <div className="item-field"><b>descricao</b><span>{item["descricao"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>curso</b><span>{item.cursoId ? ((cursoList.find(o => o.id === item.cursoId) || {}).titulo ?? ("#" + item.cursoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Modulo</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">cursoId</label>
      ...
      <select value={form["curso"] ?? ""} onChange={e => setForm({...form, curso: e.target.value})}>
        <option value="">Selecione...</option>
        {(cursoList || []).map(o => (<option key={o.id} value={o.id}>{o.titulo ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelInscricao({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["cursoId", "alunoNome", "alunoEmail", "dataInscricao", "status", "progresso", "notaFinal", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["cursoId", "alunoNome", "alunoEmail", "dataInscricao", "status", "progresso", "notaFinal"].forEach(k => { f[k] = item[k] ?? ""; });
    ["curso"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["alunoNome"] ?? "Inscricao")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["alunoNome"] ?? "Inscricao")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>cursoId</b><span>{item["cursoId"] ?? "—"}</span></div>
            <div className="item-field"><b>alunoEmail</b><span>{item["alunoEmail"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>progresso</b><span>{item["progresso"] != null ? Number(item["progresso"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>notaFinal</b><span>{item["notaFinal"] != null ? Number(item["notaFinal"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>curso</b><span>{item.cursoId ? ((cursoList.find(o => o.id === item.cursoId) || {}).titulo ?? ("#" + item.cursoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
        ...
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={salvar}>
              <label className="field-label">cursoId</label>
      <input type="number" value={form["cursoId"] ?? ""} onChange={e => setForm({...form, cursoId: e.target.value})} />
      <label className="field-label">alunoNome</label>
      <input type="text" value={form["alunoNome"] ?? ""} onChange={e => setForm({...form, alunoNome: e.target.value})} />
      <label className="field-label">alunoEmail</label>
      <input type="text" value={form["alunoEmail"] ?? ""} onChange={e => setForm({...form, alunoEmail: e.target.value})} />
      ...
      <input type="number" step="0.01" value={form["progresso"] ?? ""} onChange={e => setForm({...form, progresso: e.target.value})} />
      <label className="field-label">notaFinal</label>
      <input type="number" step="0.01" value={form["notaFinal"] ?? ""} onChange={e => setForm({...form, notaFinal: e.target.value})} />
      <label className="field-label">curso</label>
      <select value={form["curso"] ?? ""} onChange={e => setForm({...form, curso: e.target.value})}>
        <option value="">Selecione...</option>
        {(cursoList || []).map(o => (<option key={o.id} value={o.id}>{o.titulo ?? ("#" + o.id)}</option>))}
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Instrutor");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Cursos<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Instrutor" ? " active" : "")} onClick={() => setAba("Instrutor")}><span className="nav-ico">🗂️</span>Instrutor</button>
        <button className={"nav-btn" + (aba === "Curso" ? " active" : "")} onClick={() => setAba("Curso")}><span className="nav-ico">🗂️</span>Curso</button>
        <button className={"nav-btn" + (aba === "Modulo" ? " active" : "")} onClick={() => setAba("Modulo")}><span className="nav-ico">🗂️</span>Modulo</button>
      ...
      <main className="main">
        ...
        {aba === "Instrutor" && <PainelInstrutor token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Curso" && <PainelCurso token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Modulo" && <PainelModulo token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
